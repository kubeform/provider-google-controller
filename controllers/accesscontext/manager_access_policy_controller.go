...
// ManagerAccessPolicyReconciler reconciles a ManagerAccessPolicy object
type ManagerAccessPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=accesscontext.google.kubeform.com,resources=manageraccesspolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=accesscontext.google.kubeform.com,resources=manageraccesspolicies/status,verbs=get;update;patch

func (r *ManagerAccessPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("manageraccesspolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagerAccessPolicy")
...
func (r *ManagerAccessPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &accesscontextv1alpha1.ManagerAccessPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", accesscontextv1alpha1.ManagerAccessPolicy{}.APIVersion, accesscontextv1alpha1.ManagerAccessPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&accesscontextv1alpha1.ManagerAccessPolicy{}).
