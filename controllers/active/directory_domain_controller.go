...
// DirectoryDomainReconciler reconciles a DirectoryDomain object
type DirectoryDomainReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=active.google.kubeform.com,resources=directorydomains,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=active.google.kubeform.com,resources=directorydomains/status,verbs=get;update;patch

func (r *DirectoryDomainReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("directorydomain", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DirectoryDomain")
...
func (r *DirectoryDomainReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &activev1alpha1.DirectoryDomain{}); err != nil {
			klog.Error(err, "unable to set up auditor", activev1alpha1.DirectoryDomain{}.APIVersion, activev1alpha1.DirectoryDomain{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&activev1alpha1.DirectoryDomain{}).
