...
// DirectoryDomainTrustReconciler reconciles a DirectoryDomainTrust object
type DirectoryDomainTrustReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=active.google.kubeform.com,resources=directorydomaintrusts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=active.google.kubeform.com,resources=directorydomaintrusts/status,verbs=get;update;patch

func (r *DirectoryDomainTrustReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("directorydomaintrust", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DirectoryDomainTrust")
...
func (r *DirectoryDomainTrustReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &activev1alpha1.DirectoryDomainTrust{}); err != nil {
			klog.Error(err, "unable to set up auditor", activev1alpha1.DirectoryDomainTrust{}.APIVersion, activev1alpha1.DirectoryDomainTrust{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&activev1alpha1.DirectoryDomainTrust{}).
...
func (r *DirectoryDomainTrustReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for DirectoryDomainTrust")
			return nil
		}
		...
		resourceList := &activev1alpha1.DirectoryDomainTrustList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DirectoryDomainTrust")
			return nil
		}
