...
// EnvironmentIamPolicyReconciler reconciles a EnvironmentIamPolicy object
type EnvironmentIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apigee.google.kubeform.com,resources=environmentiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigee.google.kubeform.com,resources=environmentiampolicies/status,verbs=get;update;patch

func (r *EnvironmentIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("environmentiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EnvironmentIamPolicy")
...
func (r *EnvironmentIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigeev1alpha1.EnvironmentIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigeev1alpha1.EnvironmentIamPolicy{}.APIVersion, apigeev1alpha1.EnvironmentIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigeev1alpha1.EnvironmentIamPolicy{}).
