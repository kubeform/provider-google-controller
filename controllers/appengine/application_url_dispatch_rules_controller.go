...
// ApplicationURLDispatchRulesReconciler reconciles a ApplicationURLDispatchRules object
type ApplicationURLDispatchRulesReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=appengine.google.kubeform.com,resources=applicationurldispatchrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appengine.google.kubeform.com,resources=applicationurldispatchrules/status,verbs=get;update;patch

func (r *ApplicationURLDispatchRulesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("applicationurldispatchrules", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationURLDispatchRules")
...
func (r *ApplicationURLDispatchRulesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appenginev1alpha1.ApplicationURLDispatchRules{}); err != nil {
			klog.Error(err, "unable to set up auditor", appenginev1alpha1.ApplicationURLDispatchRules{}.APIVersion, appenginev1alpha1.ApplicationURLDispatchRules{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appenginev1alpha1.ApplicationURLDispatchRules{}).
