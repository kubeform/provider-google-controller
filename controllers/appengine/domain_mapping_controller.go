...
// DomainMappingReconciler reconciles a DomainMapping object
type DomainMappingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=appengine.google.kubeform.com,resources=domainmappings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appengine.google.kubeform.com,resources=domainmappings/status,verbs=get;update;patch

func (r *DomainMappingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domainmapping", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainMapping")
...
func (r *DomainMappingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appenginev1alpha1.DomainMapping{}); err != nil {
			klog.Error(err, "unable to set up auditor", appenginev1alpha1.DomainMapping{}.APIVersion, appenginev1alpha1.DomainMapping{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appenginev1alpha1.DomainMapping{}).
