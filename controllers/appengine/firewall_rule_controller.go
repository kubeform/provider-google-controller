...
// +kubebuilder:rbac:groups=appengine.google.kubeform.com,resources=firewallrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appengine.google.kubeform.com,resources=firewallrules/status,verbs=get;update;patch

func (r *FirewallRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("firewallrule", req.NamespacedName)
...
func (r *FirewallRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appenginev1alpha1.FirewallRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", appenginev1alpha1.FirewallRule{}.APIVersion, appenginev1alpha1.FirewallRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appenginev1alpha1.FirewallRule{}).
