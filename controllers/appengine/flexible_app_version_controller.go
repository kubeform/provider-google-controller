...
// FlexibleAppVersionReconciler reconciles a FlexibleAppVersion object
type FlexibleAppVersionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=appengine.google.kubeform.com,resources=flexibleappversions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appengine.google.kubeform.com,resources=flexibleappversions/status,verbs=get;update;patch

func (r *FlexibleAppVersionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("flexibleappversion", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FlexibleAppVersion")
...
func (r *FlexibleAppVersionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appenginev1alpha1.FlexibleAppVersion{}); err != nil {
			klog.Error(err, "unable to set up auditor", appenginev1alpha1.FlexibleAppVersion{}.APIVersion, appenginev1alpha1.FlexibleAppVersion{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appenginev1alpha1.FlexibleAppVersion{}).
