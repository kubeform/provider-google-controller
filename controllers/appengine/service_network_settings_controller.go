...
// ServiceNetworkSettingsReconciler reconciles a ServiceNetworkSettings object
type ServiceNetworkSettingsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=appengine.google.kubeform.com,resources=servicenetworksettings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appengine.google.kubeform.com,resources=servicenetworksettings/status,verbs=get;update;patch

func (r *ServiceNetworkSettingsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicenetworksettings", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceNetworkSettings")
...
func (r *ServiceNetworkSettingsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appenginev1alpha1.ServiceNetworkSettings{}); err != nil {
			klog.Error(err, "unable to set up auditor", appenginev1alpha1.ServiceNetworkSettings{}.APIVersion, appenginev1alpha1.ServiceNetworkSettings{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appenginev1alpha1.ServiceNetworkSettings{}).
