...
// StandardAppVersionReconciler reconciles a StandardAppVersion object
type StandardAppVersionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=appengine.google.kubeform.com,resources=standardappversions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appengine.google.kubeform.com,resources=standardappversions/status,verbs=get;update;patch

func (r *StandardAppVersionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("standardappversion", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StandardAppVersion")
...
func (r *StandardAppVersionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appenginev1alpha1.StandardAppVersion{}); err != nil {
			klog.Error(err, "unable to set up auditor", appenginev1alpha1.StandardAppVersion{}.APIVersion, appenginev1alpha1.StandardAppVersion{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appenginev1alpha1.StandardAppVersion{}).
