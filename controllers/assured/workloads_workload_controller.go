...
// WorkloadsWorkloadReconciler reconciles a WorkloadsWorkload object
type WorkloadsWorkloadReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=assured.google.kubeform.com,resources=workloadsworkloads,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=assured.google.kubeform.com,resources=workloadsworkloads/status,verbs=get;update;patch

func (r *WorkloadsWorkloadReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("workloadsworkload", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WorkloadsWorkload")
...
func (r *WorkloadsWorkloadReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &assuredv1alpha1.WorkloadsWorkload{}); err != nil {
			klog.Error(err, "unable to set up auditor", assuredv1alpha1.WorkloadsWorkload{}.APIVersion, assuredv1alpha1.WorkloadsWorkload{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&assuredv1alpha1.WorkloadsWorkload{}).
