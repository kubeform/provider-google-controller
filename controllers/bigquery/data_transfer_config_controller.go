...
// DataTransferConfigReconciler reconciles a DataTransferConfig object
type DataTransferConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bigquery.google.kubeform.com,resources=datatransferconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bigquery.google.kubeform.com,resources=datatransferconfigs/status,verbs=get;update;patch

func (r *DataTransferConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("datatransferconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DataTransferConfig")
...
func (r *DataTransferConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bigqueryv1alpha1.DataTransferConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", bigqueryv1alpha1.DataTransferConfig{}.APIVersion, bigqueryv1alpha1.DataTransferConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bigqueryv1alpha1.DataTransferConfig{}).
...
func (r *DataTransferConfigReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for DataTransferConfig")
			return nil
		}
		...
		resourceList := &bigqueryv1alpha1.DataTransferConfigList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DataTransferConfig")
			return nil
		}
