...
// DatasetAccessReconciler reconciles a DatasetAccess object
type DatasetAccessReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bigquery.google.kubeform.com,resources=datasetaccesses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bigquery.google.kubeform.com,resources=datasetaccesses/status,verbs=get;update;patch

func (r *DatasetAccessReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("datasetaccess", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DatasetAccess")
...
func (r *DatasetAccessReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bigqueryv1alpha1.DatasetAccess{}); err != nil {
			klog.Error(err, "unable to set up auditor", bigqueryv1alpha1.DatasetAccess{}.APIVersion, bigqueryv1alpha1.DatasetAccess{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bigqueryv1alpha1.DatasetAccess{}).
