...
// +kubebuilder:rbac:groups=bigquery.google.kubeform.com,resources=routines,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bigquery.google.kubeform.com,resources=routines/status,verbs=get;update;patch

func (r *RoutineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("routine", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Routine")
...
func (r *RoutineReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bigqueryv1alpha1.Routine{}); err != nil {
			klog.Error(err, "unable to set up auditor", bigqueryv1alpha1.Routine{}.APIVersion, bigqueryv1alpha1.Routine{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bigqueryv1alpha1.Routine{}).
