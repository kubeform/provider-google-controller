...
// TableIamBindingReconciler reconciles a TableIamBinding object
type TableIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bigquery.google.kubeform.com,resources=tableiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bigquery.google.kubeform.com,resources=tableiambindings/status,verbs=get;update;patch

func (r *TableIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tableiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TableIamBinding")
...
func (r *TableIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bigqueryv1alpha1.TableIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", bigqueryv1alpha1.TableIamBinding{}.APIVersion, bigqueryv1alpha1.TableIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bigqueryv1alpha1.TableIamBinding{}).
