...
// TableIamPolicyReconciler reconciles a TableIamPolicy object
type TableIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bigquery.google.kubeform.com,resources=tableiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bigquery.google.kubeform.com,resources=tableiampolicies/status,verbs=get;update;patch

func (r *TableIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tableiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TableIamPolicy")
...
func (r *TableIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bigqueryv1alpha1.TableIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", bigqueryv1alpha1.TableIamPolicy{}.APIVersion, bigqueryv1alpha1.TableIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bigqueryv1alpha1.TableIamPolicy{}).
