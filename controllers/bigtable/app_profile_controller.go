...
// +kubebuilder:rbac:groups=bigtable.google.kubeform.com,resources=appprofiles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bigtable.google.kubeform.com,resources=appprofiles/status,verbs=get;update;patch

func (r *AppProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("appprofile", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AppProfile")
...
func (r *AppProfileReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bigtablev1alpha1.AppProfile{}); err != nil {
			klog.Error(err, "unable to set up auditor", bigtablev1alpha1.AppProfile{}.APIVersion, bigtablev1alpha1.AppProfile{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bigtablev1alpha1.AppProfile{}).
