...
// GcPolicyReconciler reconciles a GcPolicy object
...
// +kubebuilder:rbac:groups=bigtable.google.kubeform.com,resources=gcpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bigtable.google.kubeform.com,resources=gcpolicies/status,verbs=get;update;patch

func (r *GcPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gcpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GcPolicy")
...
func (r *GcPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bigtablev1alpha1.GcPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", bigtablev1alpha1.GcPolicy{}.APIVersion, bigtablev1alpha1.GcPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bigtablev1alpha1.GcPolicy{}).
