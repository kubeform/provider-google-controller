...
// BillingBudgetReconciler reconciles a BillingBudget object
type BillingBudgetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=billingbudget.google.kubeform.com,resources=billingbudgets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=billingbudget.google.kubeform.com,resources=billingbudgets/status,verbs=get;update;patch

func (r *BillingBudgetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("billingbudget", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BillingBudget")
...
func (r *BillingBudgetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &billingbudgetv1alpha1.BillingBudget{}); err != nil {
			klog.Error(err, "unable to set up auditor", billingbudgetv1alpha1.BillingBudget{}.APIVersion, billingbudgetv1alpha1.BillingBudget{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&billingbudgetv1alpha1.BillingBudget{}).
