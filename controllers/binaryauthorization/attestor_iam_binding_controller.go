...
// AttestorIamBindingReconciler reconciles a AttestorIamBinding object
type AttestorIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=binaryauthorization.google.kubeform.com,resources=attestoriambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=binaryauthorization.google.kubeform.com,resources=attestoriambindings/status,verbs=get;update;patch

func (r *AttestorIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("attestoriambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AttestorIamBinding")
...
func (r *AttestorIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &binaryauthorizationv1alpha1.AttestorIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", binaryauthorizationv1alpha1.AttestorIamBinding{}.APIVersion, binaryauthorizationv1alpha1.AttestorIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&binaryauthorizationv1alpha1.AttestorIamBinding{}).
