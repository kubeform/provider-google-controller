...
// AttestorIamMemberReconciler reconciles a AttestorIamMember object
type AttestorIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=binaryauthorization.google.kubeform.com,resources=attestoriammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=binaryauthorization.google.kubeform.com,resources=attestoriammembers/status,verbs=get;update;patch

func (r *AttestorIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("attestoriammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AttestorIamMember")
...
func (r *AttestorIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &binaryauthorizationv1alpha1.AttestorIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", binaryauthorizationv1alpha1.AttestorIamMember{}.APIVersion, binaryauthorizationv1alpha1.AttestorIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&binaryauthorizationv1alpha1.AttestorIamMember{}).
