...
// AssetOrganizationFeedReconciler reconciles a AssetOrganizationFeed object
type AssetOrganizationFeedReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.google.kubeform.com,resources=assetorganizationfeeds,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.google.kubeform.com,resources=assetorganizationfeeds/status,verbs=get;update;patch

func (r *AssetOrganizationFeedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("assetorganizationfeed", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AssetOrganizationFeed")
...
func (r *AssetOrganizationFeedReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.AssetOrganizationFeed{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.AssetOrganizationFeed{}.APIVersion, cloudv1alpha1.AssetOrganizationFeed{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.AssetOrganizationFeed{}).
