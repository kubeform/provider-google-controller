...
// IdentityGroupReconciler reconciles a IdentityGroup object
type IdentityGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.google.kubeform.com,resources=identitygroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.google.kubeform.com,resources=identitygroups/status,verbs=get;update;patch

func (r *IdentityGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("identitygroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IdentityGroup")
...
func (r *IdentityGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.IdentityGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.IdentityGroup{}.APIVersion, cloudv1alpha1.IdentityGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.IdentityGroup{}).
