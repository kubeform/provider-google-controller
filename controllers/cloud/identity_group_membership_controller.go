...
// IdentityGroupMembershipReconciler reconciles a IdentityGroupMembership object
type IdentityGroupMembershipReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.google.kubeform.com,resources=identitygroupmemberships,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.google.kubeform.com,resources=identitygroupmemberships/status,verbs=get;update;patch

func (r *IdentityGroupMembershipReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("identitygroupmembership", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IdentityGroupMembership")
...
func (r *IdentityGroupMembershipReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.IdentityGroupMembership{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.IdentityGroupMembership{}.APIVersion, cloudv1alpha1.IdentityGroupMembership{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.IdentityGroupMembership{}).
