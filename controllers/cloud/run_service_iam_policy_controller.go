...
// RunServiceIamPolicyReconciler reconciles a RunServiceIamPolicy object
type RunServiceIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.google.kubeform.com,resources=runserviceiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.google.kubeform.com,resources=runserviceiampolicies/status,verbs=get;update;patch

func (r *RunServiceIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("runserviceiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RunServiceIamPolicy")
...
func (r *RunServiceIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.RunServiceIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.RunServiceIamPolicy{}.APIVersion, cloudv1alpha1.RunServiceIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.RunServiceIamPolicy{}).
