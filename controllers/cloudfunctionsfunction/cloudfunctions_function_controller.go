...
// CloudfunctionsFunctionReconciler reconciles a CloudfunctionsFunction object
type CloudfunctionsFunctionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloudfunctionsfunction.google.kubeform.com,resources=cloudfunctionsfunctions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudfunctionsfunction.google.kubeform.com,resources=cloudfunctionsfunctions/status,verbs=get;update;patch

func (r *CloudfunctionsFunctionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudfunctionsfunction", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CloudfunctionsFunction")
...
func (r *CloudfunctionsFunctionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudfunctionsfunctionv1alpha1.CloudfunctionsFunction{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudfunctionsfunctionv1alpha1.CloudfunctionsFunction{}.APIVersion, cloudfunctionsfunctionv1alpha1.CloudfunctionsFunction{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudfunctionsfunctionv1alpha1.CloudfunctionsFunction{}).
