...
// IamBindingReconciler reconciles a IamBinding object
type IamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloudfunctionsfunction.google.kubeform.com,resources=iambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudfunctionsfunction.google.kubeform.com,resources=iambindings/status,verbs=get;update;patch

func (r *IamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("iambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IamBinding")
...
func (r *IamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudfunctionsfunctionv1alpha1.IamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudfunctionsfunctionv1alpha1.IamBinding{}.APIVersion, cloudfunctionsfunctionv1alpha1.IamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudfunctionsfunctionv1alpha1.IamBinding{}).
