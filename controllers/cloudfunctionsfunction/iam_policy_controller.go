...
// +kubebuilder:rbac:groups=cloudfunctionsfunction.google.kubeform.com,resources=iampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudfunctionsfunction.google.kubeform.com,resources=iampolicies/status,verbs=get;update;patch

func (r *IamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("iampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IamPolicy")
...
func (r *IamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudfunctionsfunctionv1alpha1.IamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudfunctionsfunctionv1alpha1.IamPolicy{}.APIVersion, cloudfunctionsfunctionv1alpha1.IamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudfunctionsfunctionv1alpha1.IamPolicy{}).
