...
// BackendBucketSignedURLKeyReconciler reconciles a BackendBucketSignedURLKey object
type BackendBucketSignedURLKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=backendbucketsignedurlkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=backendbucketsignedurlkeys/status,verbs=get;update;patch

func (r *BackendBucketSignedURLKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("backendbucketsignedurlkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BackendBucketSignedURLKey")
...
func (r *BackendBucketSignedURLKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.BackendBucketSignedURLKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.BackendBucketSignedURLKey{}.APIVersion, computev1alpha1.BackendBucketSignedURLKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.BackendBucketSignedURLKey{}).
...
func (r *BackendBucketSignedURLKeyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for BackendBucketSignedURLKey")
			return nil
		}
		...
		resourceList := &computev1alpha1.BackendBucketSignedURLKeyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list BackendBucketSignedURLKey")
			return nil
		}
