...
// BackendServiceSignedURLKeyReconciler reconciles a BackendServiceSignedURLKey object
type BackendServiceSignedURLKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=backendservicesignedurlkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=backendservicesignedurlkeys/status,verbs=get;update;patch

func (r *BackendServiceSignedURLKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("backendservicesignedurlkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BackendServiceSignedURLKey")
...
func (r *BackendServiceSignedURLKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.BackendServiceSignedURLKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.BackendServiceSignedURLKey{}.APIVersion, computev1alpha1.BackendServiceSignedURLKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.BackendServiceSignedURLKey{}).
...
func (r *BackendServiceSignedURLKeyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for BackendServiceSignedURLKey")
			return nil
		}
		...
		resourceList := &computev1alpha1.BackendServiceSignedURLKeyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list BackendServiceSignedURLKey")
			return nil
		}
