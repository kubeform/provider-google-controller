...
// GlobalNetworkEndpointGroupReconciler reconciles a GlobalNetworkEndpointGroup object
type GlobalNetworkEndpointGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=globalnetworkendpointgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=globalnetworkendpointgroups/status,verbs=get;update;patch

func (r *GlobalNetworkEndpointGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("globalnetworkendpointgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GlobalNetworkEndpointGroup")
...
func (r *GlobalNetworkEndpointGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.GlobalNetworkEndpointGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.GlobalNetworkEndpointGroup{}.APIVersion, computev1alpha1.GlobalNetworkEndpointGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.GlobalNetworkEndpointGroup{}).
