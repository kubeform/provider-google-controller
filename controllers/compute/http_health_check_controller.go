...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=httphealthchecks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=httphealthchecks/status,verbs=get;update;patch

func (r *HttpHealthCheckReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("httphealthcheck", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HttpHealthCheck")
...
func (r *HttpHealthCheckReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.HttpHealthCheck{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.HttpHealthCheck{}.APIVersion, computev1alpha1.HttpHealthCheck{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.HttpHealthCheck{}).
