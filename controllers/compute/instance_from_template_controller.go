...
// InstanceFromTemplateReconciler reconciles a InstanceFromTemplate object
type InstanceFromTemplateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=instancefromtemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=instancefromtemplates/status,verbs=get;update;patch

func (r *InstanceFromTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("instancefromtemplate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InstanceFromTemplate")
...
func (r *InstanceFromTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.InstanceFromTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.InstanceFromTemplate{}.APIVersion, computev1alpha1.InstanceFromTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.InstanceFromTemplate{}).
...
func (r *InstanceFromTemplateReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for InstanceFromTemplate")
			return nil
		}
		...
		resourceList := &computev1alpha1.InstanceFromTemplateList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list InstanceFromTemplate")
			return nil
		}
