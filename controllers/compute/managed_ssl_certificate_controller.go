...
// ManagedSslCertificateReconciler reconciles a ManagedSslCertificate object
type ManagedSslCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=managedsslcertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=managedsslcertificates/status,verbs=get;update;patch

func (r *ManagedSslCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managedsslcertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagedSslCertificate")
...
func (r *ManagedSslCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.ManagedSslCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.ManagedSslCertificate{}.APIVersion, computev1alpha1.ManagedSslCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.ManagedSslCertificate{}).
