...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=networkendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=networkendpoints/status,verbs=get;update;patch

func (r *NetworkEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkEndpoint")
...
func (r *NetworkEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.NetworkEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.NetworkEndpoint{}.APIVersion, computev1alpha1.NetworkEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.NetworkEndpoint{}).
