...
// NetworkPeeringReconciler reconciles a NetworkPeering object
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=networkpeerings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=networkpeerings/status,verbs=get;update;patch

func (r *NetworkPeeringReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkpeering", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkPeering")
...
func (r *NetworkPeeringReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.NetworkPeering{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.NetworkPeering{}.APIVersion, computev1alpha1.NetworkPeering{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.NetworkPeering{}).
