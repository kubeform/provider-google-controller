...
// NodeTemplateReconciler reconciles a NodeTemplate object
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=nodetemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=nodetemplates/status,verbs=get;update;patch

func (r *NodeTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("nodetemplate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NodeTemplate")
...
func (r *NodeTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.NodeTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.NodeTemplate{}.APIVersion, computev1alpha1.NodeTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.NodeTemplate{}).
