...
// ProjectMetadataItemReconciler reconciles a ProjectMetadataItem object
type ProjectMetadataItemReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=projectmetadataitems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=projectmetadataitems/status,verbs=get;update;patch

func (r *ProjectMetadataItemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("projectmetadataitem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProjectMetadataItem")
...
func (r *ProjectMetadataItemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.ProjectMetadataItem{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.ProjectMetadataItem{}.APIVersion, computev1alpha1.ProjectMetadataItem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.ProjectMetadataItem{}).
