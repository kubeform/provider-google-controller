...
// RegionDiskIamBindingReconciler reconciles a RegionDiskIamBinding object
type RegionDiskIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=regiondiskiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=regiondiskiambindings/status,verbs=get;update;patch

func (r *RegionDiskIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("regiondiskiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RegionDiskIamBinding")
...
func (r *RegionDiskIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.RegionDiskIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.RegionDiskIamBinding{}.APIVersion, computev1alpha1.RegionDiskIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.RegionDiskIamBinding{}).
