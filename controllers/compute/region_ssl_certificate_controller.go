...
// RegionSslCertificateReconciler reconciles a RegionSslCertificate object
type RegionSslCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=regionsslcertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=regionsslcertificates/status,verbs=get;update;patch

func (r *RegionSslCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("regionsslcertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RegionSslCertificate")
...
func (r *RegionSslCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.RegionSslCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.RegionSslCertificate{}.APIVersion, computev1alpha1.RegionSslCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.RegionSslCertificate{}).
...
func (r *RegionSslCertificateReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for RegionSslCertificate")
			return nil
		}
		...
		resourceList := &computev1alpha1.RegionSslCertificateList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list RegionSslCertificate")
			return nil
		}
