...
// RegionTargetHTTPSProxyReconciler reconciles a RegionTargetHTTPSProxy object
type RegionTargetHTTPSProxyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=regiontargethttpsproxies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=regiontargethttpsproxies/status,verbs=get;update;patch

func (r *RegionTargetHTTPSProxyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("regiontargethttpsproxy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RegionTargetHTTPSProxy")
...
func (r *RegionTargetHTTPSProxyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.RegionTargetHTTPSProxy{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.RegionTargetHTTPSProxy{}.APIVersion, computev1alpha1.RegionTargetHTTPSProxy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.RegionTargetHTTPSProxy{}).
