...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=routers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=routers/status,verbs=get;update;patch

func (r *RouterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("router", req.NamespacedName)
...
func (r *RouterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.Router{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.Router{}.APIVersion, computev1alpha1.Router{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.Router{}).
