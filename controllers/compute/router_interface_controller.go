...
// RouterInterfaceReconciler reconciles a RouterInterface object
type RouterInterfaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=routerinterfaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=routerinterfaces/status,verbs=get;update;patch

func (r *RouterInterfaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("routerinterface", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RouterInterface")
...
func (r *RouterInterfaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.RouterInterface{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.RouterInterface{}.APIVersion, computev1alpha1.RouterInterface{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.RouterInterface{}).
