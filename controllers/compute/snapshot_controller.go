...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=snapshots,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=snapshots/status,verbs=get;update;patch
...
func (r *SnapshotReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.Snapshot{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.Snapshot{}.APIVersion, computev1alpha1.Snapshot{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.Snapshot{}).
...
func (r *SnapshotReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Snapshot")
			return nil
		}
		...
		resourceList := &computev1alpha1.SnapshotList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Snapshot")
			return nil
		}
