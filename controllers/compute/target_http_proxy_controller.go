...
// TargetHTTPProxyReconciler reconciles a TargetHTTPProxy object
type TargetHTTPProxyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=targethttpproxies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.google.kubeform.com,resources=targethttpproxies/status,verbs=get;update;patch

func (r *TargetHTTPProxyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("targethttpproxy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TargetHTTPProxy")
...
func (r *TargetHTTPProxyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.TargetHTTPProxy{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.TargetHTTPProxy{}.APIVersion, computev1alpha1.TargetHTTPProxy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.TargetHTTPProxy{}).
