...
// AwsNodePoolReconciler reconciles a AwsNodePool object
type AwsNodePoolReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=container.google.kubeform.com,resources=awsnodepools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=container.google.kubeform.com,resources=awsnodepools/status,verbs=get;update;patch

func (r *AwsNodePoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("awsnodepool", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AwsNodePool")
...
func (r *AwsNodePoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &containerv1alpha1.AwsNodePool{}); err != nil {
			klog.Error(err, "unable to set up auditor", containerv1alpha1.AwsNodePool{}.APIVersion, containerv1alpha1.AwsNodePool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&containerv1alpha1.AwsNodePool{}).
