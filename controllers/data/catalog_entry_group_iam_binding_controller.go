...
// CatalogEntryGroupIamBindingReconciler reconciles a CatalogEntryGroupIamBinding object
type CatalogEntryGroupIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.google.kubeform.com,resources=catalogentrygroupiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.google.kubeform.com,resources=catalogentrygroupiambindings/status,verbs=get;update;patch

func (r *CatalogEntryGroupIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("catalogentrygroupiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CatalogEntryGroupIamBinding")
...
func (r *CatalogEntryGroupIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.CatalogEntryGroupIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.CatalogEntryGroupIamBinding{}.APIVersion, datav1alpha1.CatalogEntryGroupIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.CatalogEntryGroupIamBinding{}).
