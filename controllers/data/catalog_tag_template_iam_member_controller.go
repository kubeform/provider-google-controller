...
// CatalogTagTemplateIamMemberReconciler reconciles a CatalogTagTemplateIamMember object
type CatalogTagTemplateIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.google.kubeform.com,resources=catalogtagtemplateiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.google.kubeform.com,resources=catalogtagtemplateiammembers/status,verbs=get;update;patch

func (r *CatalogTagTemplateIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("catalogtagtemplateiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CatalogTagTemplateIamMember")
...
func (r *CatalogTagTemplateIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.CatalogTagTemplateIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.CatalogTagTemplateIamMember{}.APIVersion, datav1alpha1.CatalogTagTemplateIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.CatalogTagTemplateIamMember{}).
