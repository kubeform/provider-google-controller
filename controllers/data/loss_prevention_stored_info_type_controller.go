...
// LossPreventionStoredInfoTypeReconciler reconciles a LossPreventionStoredInfoType object
type LossPreventionStoredInfoTypeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.google.kubeform.com,resources=losspreventionstoredinfotypes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.google.kubeform.com,resources=losspreventionstoredinfotypes/status,verbs=get;update;patch

func (r *LossPreventionStoredInfoTypeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("losspreventionstoredinfotype", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LossPreventionStoredInfoType")
...
func (r *LossPreventionStoredInfoTypeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.LossPreventionStoredInfoType{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.LossPreventionStoredInfoType{}.APIVersion, datav1alpha1.LossPreventionStoredInfoType{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.LossPreventionStoredInfoType{}).
