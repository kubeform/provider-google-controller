...
// +kubebuilder:rbac:groups=dataproc.google.kubeform.com,resources=autoscalingpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dataproc.google.kubeform.com,resources=autoscalingpolicies/status,verbs=get;update;patch

func (r *AutoscalingPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("autoscalingpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AutoscalingPolicy")
...
func (r *AutoscalingPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dataprocv1alpha1.AutoscalingPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", dataprocv1alpha1.AutoscalingPolicy{}.APIVersion, dataprocv1alpha1.AutoscalingPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dataprocv1alpha1.AutoscalingPolicy{}).
