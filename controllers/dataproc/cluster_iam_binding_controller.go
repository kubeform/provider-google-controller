...
// ClusterIamBindingReconciler reconciles a ClusterIamBinding object
type ClusterIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dataproc.google.kubeform.com,resources=clusteriambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dataproc.google.kubeform.com,resources=clusteriambindings/status,verbs=get;update;patch

func (r *ClusterIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clusteriambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterIamBinding")
...
func (r *ClusterIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dataprocv1alpha1.ClusterIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", dataprocv1alpha1.ClusterIamBinding{}.APIVersion, dataprocv1alpha1.ClusterIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dataprocv1alpha1.ClusterIamBinding{}).
