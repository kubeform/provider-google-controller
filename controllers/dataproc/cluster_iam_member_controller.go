...
// ClusterIamMemberReconciler reconciles a ClusterIamMember object
type ClusterIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dataproc.google.kubeform.com,resources=clusteriammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dataproc.google.kubeform.com,resources=clusteriammembers/status,verbs=get;update;patch

func (r *ClusterIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clusteriammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterIamMember")
...
func (r *ClusterIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dataprocv1alpha1.ClusterIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", dataprocv1alpha1.ClusterIamMember{}.APIVersion, dataprocv1alpha1.ClusterIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dataprocv1alpha1.ClusterIamMember{}).
