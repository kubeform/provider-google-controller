...
// ClusterIamPolicyReconciler reconciles a ClusterIamPolicy object
type ClusterIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dataproc.google.kubeform.com,resources=clusteriampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dataproc.google.kubeform.com,resources=clusteriampolicies/status,verbs=get;update;patch

func (r *ClusterIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clusteriampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterIamPolicy")
...
func (r *ClusterIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dataprocv1alpha1.ClusterIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", dataprocv1alpha1.ClusterIamPolicy{}.APIVersion, dataprocv1alpha1.ClusterIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dataprocv1alpha1.ClusterIamPolicy{}).
