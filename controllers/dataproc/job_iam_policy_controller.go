...
// JobIamPolicyReconciler reconciles a JobIamPolicy object
type JobIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dataproc.google.kubeform.com,resources=jobiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dataproc.google.kubeform.com,resources=jobiampolicies/status,verbs=get;update;patch

func (r *JobIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("jobiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch JobIamPolicy")
...
func (r *JobIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dataprocv1alpha1.JobIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", dataprocv1alpha1.JobIamPolicy{}.APIVersion, dataprocv1alpha1.JobIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dataprocv1alpha1.JobIamPolicy{}).
