...
// ManagerDeploymentReconciler reconciles a ManagerDeployment object
type ManagerDeploymentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=deployment.google.kubeform.com,resources=managerdeployments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=deployment.google.kubeform.com,resources=managerdeployments/status,verbs=get;update;patch

func (r *ManagerDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managerdeployment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagerDeployment")
...
func (r *ManagerDeploymentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &deploymentv1alpha1.ManagerDeployment{}); err != nil {
			klog.Error(err, "unable to set up auditor", deploymentv1alpha1.ManagerDeployment{}.APIVersion, deploymentv1alpha1.ManagerDeployment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&deploymentv1alpha1.ManagerDeployment{}).
