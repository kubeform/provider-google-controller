...
// CxAgentReconciler reconciles a CxAgent object
type CxAgentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dialogflow.google.kubeform.com,resources=cxagents,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dialogflow.google.kubeform.com,resources=cxagents/status,verbs=get;update;patch

func (r *CxAgentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cxagent", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CxAgent")
...
func (r *CxAgentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dialogflowv1alpha1.CxAgent{}); err != nil {
			klog.Error(err, "unable to set up auditor", dialogflowv1alpha1.CxAgent{}.APIVersion, dialogflowv1alpha1.CxAgent{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dialogflowv1alpha1.CxAgent{}).
