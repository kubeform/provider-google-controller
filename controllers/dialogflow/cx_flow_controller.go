...
// CxFlowReconciler reconciles a CxFlow object
type CxFlowReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dialogflow.google.kubeform.com,resources=cxflows,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dialogflow.google.kubeform.com,resources=cxflows/status,verbs=get;update;patch

func (r *CxFlowReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cxflow", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CxFlow")
...
func (r *CxFlowReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dialogflowv1alpha1.CxFlow{}); err != nil {
			klog.Error(err, "unable to set up auditor", dialogflowv1alpha1.CxFlow{}.APIVersion, dialogflowv1alpha1.CxFlow{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dialogflowv1alpha1.CxFlow{}).
