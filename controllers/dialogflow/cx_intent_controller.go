...
// CxIntentReconciler reconciles a CxIntent object
type CxIntentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dialogflow.google.kubeform.com,resources=cxintents,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dialogflow.google.kubeform.com,resources=cxintents/status,verbs=get;update;patch

func (r *CxIntentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cxintent", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CxIntent")
...
func (r *CxIntentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dialogflowv1alpha1.CxIntent{}); err != nil {
			klog.Error(err, "unable to set up auditor", dialogflowv1alpha1.CxIntent{}.APIVersion, dialogflowv1alpha1.CxIntent{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dialogflowv1alpha1.CxIntent{}).
