...
// EntityTypeReconciler reconciles a EntityType object
type EntityTypeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dialogflow.google.kubeform.com,resources=entitytypes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dialogflow.google.kubeform.com,resources=entitytypes/status,verbs=get;update;patch

func (r *EntityTypeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("entitytype", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EntityType")
...
func (r *EntityTypeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dialogflowv1alpha1.EntityType{}); err != nil {
			klog.Error(err, "unable to set up auditor", dialogflowv1alpha1.EntityType{}.APIVersion, dialogflowv1alpha1.EntityType{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dialogflowv1alpha1.EntityType{}).
