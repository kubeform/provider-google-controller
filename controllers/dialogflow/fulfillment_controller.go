...
// FulfillmentReconciler reconciles a Fulfillment object
...
// +kubebuilder:rbac:groups=dialogflow.google.kubeform.com,resources=fulfillments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dialogflow.google.kubeform.com,resources=fulfillments/status,verbs=get;update;patch

func (r *FulfillmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fulfillment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Fulfillment")
...
func (r *FulfillmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dialogflowv1alpha1.Fulfillment{}); err != nil {
			klog.Error(err, "unable to set up auditor", dialogflowv1alpha1.Fulfillment{}.APIVersion, dialogflowv1alpha1.Fulfillment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dialogflowv1alpha1.Fulfillment{}).
