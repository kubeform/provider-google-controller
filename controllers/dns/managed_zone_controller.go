...
// +kubebuilder:rbac:groups=dns.google.kubeform.com,resources=managedzones,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.google.kubeform.com,resources=managedzones/status,verbs=get;update;patch

func (r *ManagedZoneReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managedzone", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagedZone")
...
func (r *ManagedZoneReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.ManagedZone{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.ManagedZone{}.APIVersion, dnsv1alpha1.ManagedZone{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dnsv1alpha1.ManagedZone{}).
