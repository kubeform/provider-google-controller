...
// ServiceIamBindingReconciler reconciles a ServiceIamBinding object
type ServiceIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=endpoints.google.kubeform.com,resources=serviceiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=endpoints.google.kubeform.com,resources=serviceiambindings/status,verbs=get;update;patch

func (r *ServiceIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serviceiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceIamBinding")
...
func (r *ServiceIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &endpointsv1alpha1.ServiceIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", endpointsv1alpha1.ServiceIamBinding{}.APIVersion, endpointsv1alpha1.ServiceIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&endpointsv1alpha1.ServiceIamBinding{}).
