...
// ServiceIamMemberReconciler reconciles a ServiceIamMember object
type ServiceIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=endpoints.google.kubeform.com,resources=serviceiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=endpoints.google.kubeform.com,resources=serviceiammembers/status,verbs=get;update;patch

func (r *ServiceIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serviceiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceIamMember")
...
func (r *ServiceIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &endpointsv1alpha1.ServiceIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", endpointsv1alpha1.ServiceIamMember{}.APIVersion, endpointsv1alpha1.ServiceIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&endpointsv1alpha1.ServiceIamMember{}).
