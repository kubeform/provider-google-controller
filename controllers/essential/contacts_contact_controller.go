...
// ContactsContactReconciler reconciles a ContactsContact object
type ContactsContactReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=essential.google.kubeform.com,resources=contactscontacts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=essential.google.kubeform.com,resources=contactscontacts/status,verbs=get;update;patch

func (r *ContactsContactReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("contactscontact", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ContactsContact")
...
func (r *ContactsContactReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &essentialv1alpha1.ContactsContact{}); err != nil {
			klog.Error(err, "unable to set up auditor", essentialv1alpha1.ContactsContact{}.APIVersion, essentialv1alpha1.ContactsContact{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&essentialv1alpha1.ContactsContact{}).
