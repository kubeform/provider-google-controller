...
// ServicesGameServerConfigReconciler reconciles a ServicesGameServerConfig object
type ServicesGameServerConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=game.google.kubeform.com,resources=servicesgameserverconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=game.google.kubeform.com,resources=servicesgameserverconfigs/status,verbs=get;update;patch

func (r *ServicesGameServerConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicesgameserverconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServicesGameServerConfig")
...
func (r *ServicesGameServerConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &gamev1alpha1.ServicesGameServerConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", gamev1alpha1.ServicesGameServerConfig{}.APIVersion, gamev1alpha1.ServicesGameServerConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&gamev1alpha1.ServicesGameServerConfig{}).
