...
// ServicesRealmReconciler reconciles a ServicesRealm object
type ServicesRealmReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=game.google.kubeform.com,resources=servicesrealms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=game.google.kubeform.com,resources=servicesrealms/status,verbs=get;update;patch

func (r *ServicesRealmReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicesrealm", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServicesRealm")
...
func (r *ServicesRealmReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &gamev1alpha1.ServicesRealm{}); err != nil {
			klog.Error(err, "unable to set up auditor", gamev1alpha1.ServicesRealm{}.APIVersion, gamev1alpha1.ServicesRealm{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&gamev1alpha1.ServicesRealm{}).
