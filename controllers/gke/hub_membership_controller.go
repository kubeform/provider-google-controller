...
// HubMembershipReconciler reconciles a HubMembership object
type HubMembershipReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=gke.google.kubeform.com,resources=hubmemberships,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=gke.google.kubeform.com,resources=hubmemberships/status,verbs=get;update;patch

func (r *HubMembershipReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hubmembership", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HubMembership")
...
func (r *HubMembershipReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &gkev1alpha1.HubMembership{}); err != nil {
			klog.Error(err, "unable to set up auditor", gkev1alpha1.HubMembership{}.APIVersion, gkev1alpha1.HubMembership{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&gkev1alpha1.HubMembership{}).
