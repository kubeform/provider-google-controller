...
// ConsentStoreReconciler reconciles a ConsentStore object
type ConsentStoreReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=consentstores,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=consentstores/status,verbs=get;update;patch

func (r *ConsentStoreReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("consentstore", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConsentStore")
...
func (r *ConsentStoreReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &healthcarev1alpha1.ConsentStore{}); err != nil {
			klog.Error(err, "unable to set up auditor", healthcarev1alpha1.ConsentStore{}.APIVersion, healthcarev1alpha1.ConsentStore{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&healthcarev1alpha1.ConsentStore{}).
