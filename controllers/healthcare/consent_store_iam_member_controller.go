...
// ConsentStoreIamMemberReconciler reconciles a ConsentStoreIamMember object
type ConsentStoreIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=consentstoreiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=consentstoreiammembers/status,verbs=get;update;patch

func (r *ConsentStoreIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("consentstoreiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConsentStoreIamMember")
...
func (r *ConsentStoreIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &healthcarev1alpha1.ConsentStoreIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", healthcarev1alpha1.ConsentStoreIamMember{}.APIVersion, healthcarev1alpha1.ConsentStoreIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&healthcarev1alpha1.ConsentStoreIamMember{}).
