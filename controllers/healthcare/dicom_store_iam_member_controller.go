...
// DicomStoreIamMemberReconciler reconciles a DicomStoreIamMember object
type DicomStoreIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=dicomstoreiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=dicomstoreiammembers/status,verbs=get;update;patch

func (r *DicomStoreIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dicomstoreiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DicomStoreIamMember")
...
func (r *DicomStoreIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &healthcarev1alpha1.DicomStoreIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", healthcarev1alpha1.DicomStoreIamMember{}.APIVersion, healthcarev1alpha1.DicomStoreIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&healthcarev1alpha1.DicomStoreIamMember{}).
