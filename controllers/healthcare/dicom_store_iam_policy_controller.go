...
// DicomStoreIamPolicyReconciler reconciles a DicomStoreIamPolicy object
type DicomStoreIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=dicomstoreiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=dicomstoreiampolicies/status,verbs=get;update;patch

func (r *DicomStoreIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dicomstoreiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DicomStoreIamPolicy")
...
func (r *DicomStoreIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &healthcarev1alpha1.DicomStoreIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", healthcarev1alpha1.DicomStoreIamPolicy{}.APIVersion, healthcarev1alpha1.DicomStoreIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&healthcarev1alpha1.DicomStoreIamPolicy{}).
