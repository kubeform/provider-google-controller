...
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=fhirstores,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=fhirstores/status,verbs=get;update;patch

func (r *FhirStoreReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fhirstore", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FhirStore")
...
func (r *FhirStoreReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &healthcarev1alpha1.FhirStore{}); err != nil {
			klog.Error(err, "unable to set up auditor", healthcarev1alpha1.FhirStore{}.APIVersion, healthcarev1alpha1.FhirStore{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&healthcarev1alpha1.FhirStore{}).
