...
// FhirStoreIamBindingReconciler reconciles a FhirStoreIamBinding object
type FhirStoreIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=fhirstoreiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=fhirstoreiambindings/status,verbs=get;update;patch

func (r *FhirStoreIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fhirstoreiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FhirStoreIamBinding")
...
func (r *FhirStoreIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &healthcarev1alpha1.FhirStoreIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", healthcarev1alpha1.FhirStoreIamBinding{}.APIVersion, healthcarev1alpha1.FhirStoreIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&healthcarev1alpha1.FhirStoreIamBinding{}).
