...
// FhirStoreIamMemberReconciler reconciles a FhirStoreIamMember object
type FhirStoreIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=fhirstoreiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=fhirstoreiammembers/status,verbs=get;update;patch

func (r *FhirStoreIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fhirstoreiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FhirStoreIamMember")
...
func (r *FhirStoreIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &healthcarev1alpha1.FhirStoreIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", healthcarev1alpha1.FhirStoreIamMember{}.APIVersion, healthcarev1alpha1.FhirStoreIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&healthcarev1alpha1.FhirStoreIamMember{}).
