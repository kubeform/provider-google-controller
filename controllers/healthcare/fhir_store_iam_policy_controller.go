...
// FhirStoreIamPolicyReconciler reconciles a FhirStoreIamPolicy object
type FhirStoreIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=fhirstoreiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=healthcare.google.kubeform.com,resources=fhirstoreiampolicies/status,verbs=get;update;patch

func (r *FhirStoreIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fhirstoreiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FhirStoreIamPolicy")
...
func (r *FhirStoreIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &healthcarev1alpha1.FhirStoreIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", healthcarev1alpha1.FhirStoreIamPolicy{}.APIVersion, healthcarev1alpha1.FhirStoreIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&healthcarev1alpha1.FhirStoreIamPolicy{}).
