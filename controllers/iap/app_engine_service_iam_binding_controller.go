...
// AppEngineServiceIamBindingReconciler reconciles a AppEngineServiceIamBinding object
type AppEngineServiceIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=appengineserviceiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=appengineserviceiambindings/status,verbs=get;update;patch

func (r *AppEngineServiceIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("appengineserviceiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AppEngineServiceIamBinding")
...
func (r *AppEngineServiceIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iapv1alpha1.AppEngineServiceIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", iapv1alpha1.AppEngineServiceIamBinding{}.APIVersion, iapv1alpha1.AppEngineServiceIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iapv1alpha1.AppEngineServiceIamBinding{}).
