...
// AppEngineServiceIamPolicyReconciler reconciles a AppEngineServiceIamPolicy object
type AppEngineServiceIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=appengineserviceiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=appengineserviceiampolicies/status,verbs=get;update;patch

func (r *AppEngineServiceIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("appengineserviceiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AppEngineServiceIamPolicy")
...
func (r *AppEngineServiceIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iapv1alpha1.AppEngineServiceIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", iapv1alpha1.AppEngineServiceIamPolicy{}.APIVersion, iapv1alpha1.AppEngineServiceIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iapv1alpha1.AppEngineServiceIamPolicy{}).
