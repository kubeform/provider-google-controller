...
// TunnelInstanceIamBindingReconciler reconciles a TunnelInstanceIamBinding object
type TunnelInstanceIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=tunnelinstanceiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=tunnelinstanceiambindings/status,verbs=get;update;patch

func (r *TunnelInstanceIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tunnelinstanceiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TunnelInstanceIamBinding")
...
func (r *TunnelInstanceIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iapv1alpha1.TunnelInstanceIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", iapv1alpha1.TunnelInstanceIamBinding{}.APIVersion, iapv1alpha1.TunnelInstanceIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iapv1alpha1.TunnelInstanceIamBinding{}).
