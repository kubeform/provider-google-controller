...
// TunnelInstanceIamPolicyReconciler reconciles a TunnelInstanceIamPolicy object
type TunnelInstanceIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=tunnelinstanceiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=tunnelinstanceiampolicies/status,verbs=get;update;patch

func (r *TunnelInstanceIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tunnelinstanceiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TunnelInstanceIamPolicy")
...
func (r *TunnelInstanceIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iapv1alpha1.TunnelInstanceIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", iapv1alpha1.TunnelInstanceIamPolicy{}.APIVersion, iapv1alpha1.TunnelInstanceIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iapv1alpha1.TunnelInstanceIamPolicy{}).
