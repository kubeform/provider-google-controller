...
// WebBackendServiceIamMemberReconciler reconciles a WebBackendServiceIamMember object
type WebBackendServiceIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=webbackendserviceiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=webbackendserviceiammembers/status,verbs=get;update;patch

func (r *WebBackendServiceIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("webbackendserviceiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WebBackendServiceIamMember")
...
func (r *WebBackendServiceIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iapv1alpha1.WebBackendServiceIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", iapv1alpha1.WebBackendServiceIamMember{}.APIVersion, iapv1alpha1.WebBackendServiceIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iapv1alpha1.WebBackendServiceIamMember{}).
