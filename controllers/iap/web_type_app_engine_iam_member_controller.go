...
// WebTypeAppEngineIamMemberReconciler reconciles a WebTypeAppEngineIamMember object
type WebTypeAppEngineIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=webtypeappengineiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=webtypeappengineiammembers/status,verbs=get;update;patch

func (r *WebTypeAppEngineIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("webtypeappengineiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WebTypeAppEngineIamMember")
...
func (r *WebTypeAppEngineIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iapv1alpha1.WebTypeAppEngineIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", iapv1alpha1.WebTypeAppEngineIamMember{}.APIVersion, iapv1alpha1.WebTypeAppEngineIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iapv1alpha1.WebTypeAppEngineIamMember{}).
