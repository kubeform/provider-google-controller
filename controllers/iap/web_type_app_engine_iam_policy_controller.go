...
// WebTypeAppEngineIamPolicyReconciler reconciles a WebTypeAppEngineIamPolicy object
type WebTypeAppEngineIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=webtypeappengineiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iap.google.kubeform.com,resources=webtypeappengineiampolicies/status,verbs=get;update;patch

func (r *WebTypeAppEngineIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("webtypeappengineiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WebTypeAppEngineIamPolicy")
...
func (r *WebTypeAppEngineIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iapv1alpha1.WebTypeAppEngineIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", iapv1alpha1.WebTypeAppEngineIamPolicy{}.APIVersion, iapv1alpha1.WebTypeAppEngineIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iapv1alpha1.WebTypeAppEngineIamPolicy{}).
