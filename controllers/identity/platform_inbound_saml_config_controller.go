...
// PlatformInboundSamlConfigReconciler reconciles a PlatformInboundSamlConfig object
type PlatformInboundSamlConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=identity.google.kubeform.com,resources=platforminboundsamlconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.google.kubeform.com,resources=platforminboundsamlconfigs/status,verbs=get;update;patch

func (r *PlatformInboundSamlConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("platforminboundsamlconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PlatformInboundSamlConfig")
...
func (r *PlatformInboundSamlConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &identityv1alpha1.PlatformInboundSamlConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", identityv1alpha1.PlatformInboundSamlConfig{}.APIVersion, identityv1alpha1.PlatformInboundSamlConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&identityv1alpha1.PlatformInboundSamlConfig{}).
