...
// PlatformOauthIdpConfigReconciler reconciles a PlatformOauthIdpConfig object
type PlatformOauthIdpConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=identity.google.kubeform.com,resources=platformoauthidpconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.google.kubeform.com,resources=platformoauthidpconfigs/status,verbs=get;update;patch

func (r *PlatformOauthIdpConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("platformoauthidpconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PlatformOauthIdpConfig")
...
func (r *PlatformOauthIdpConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &identityv1alpha1.PlatformOauthIdpConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", identityv1alpha1.PlatformOauthIdpConfig{}.APIVersion, identityv1alpha1.PlatformOauthIdpConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&identityv1alpha1.PlatformOauthIdpConfig{}).
