...
// +kubebuilder:rbac:groups=identity.google.kubeform.com,resources=platformtenants,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.google.kubeform.com,resources=platformtenants/status,verbs=get;update;patch

func (r *PlatformTenantReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("platformtenant", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PlatformTenant")
...
func (r *PlatformTenantReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &identityv1alpha1.PlatformTenant{}); err != nil {
			klog.Error(err, "unable to set up auditor", identityv1alpha1.PlatformTenant{}.APIVersion, identityv1alpha1.PlatformTenant{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&identityv1alpha1.PlatformTenant{}).
