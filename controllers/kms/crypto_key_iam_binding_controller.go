...
// CryptoKeyIamBindingReconciler reconciles a CryptoKeyIamBinding object
type CryptoKeyIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.google.kubeform.com,resources=cryptokeyiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.google.kubeform.com,resources=cryptokeyiambindings/status,verbs=get;update;patch

func (r *CryptoKeyIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cryptokeyiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CryptoKeyIamBinding")
...
func (r *CryptoKeyIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.CryptoKeyIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.CryptoKeyIamBinding{}.APIVersion, kmsv1alpha1.CryptoKeyIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.CryptoKeyIamBinding{}).
