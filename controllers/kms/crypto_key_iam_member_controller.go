...
// CryptoKeyIamMemberReconciler reconciles a CryptoKeyIamMember object
type CryptoKeyIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.google.kubeform.com,resources=cryptokeyiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.google.kubeform.com,resources=cryptokeyiammembers/status,verbs=get;update;patch

func (r *CryptoKeyIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cryptokeyiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CryptoKeyIamMember")
...
func (r *CryptoKeyIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.CryptoKeyIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.CryptoKeyIamMember{}.APIVersion, kmsv1alpha1.CryptoKeyIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.CryptoKeyIamMember{}).
