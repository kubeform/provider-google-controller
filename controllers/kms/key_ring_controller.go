...
// KeyRingReconciler reconciles a KeyRing object
...
// +kubebuilder:rbac:groups=kms.google.kubeform.com,resources=keyrings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.google.kubeform.com,resources=keyrings/status,verbs=get;update;patch

func (r *KeyRingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("keyring", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch KeyRing")
...
func (r *KeyRingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.KeyRing{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.KeyRing{}.APIVersion, kmsv1alpha1.KeyRing{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.KeyRing{}).
