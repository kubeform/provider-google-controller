...
// KeyRingIamPolicyReconciler reconciles a KeyRingIamPolicy object
type KeyRingIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.google.kubeform.com,resources=keyringiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.google.kubeform.com,resources=keyringiampolicies/status,verbs=get;update;patch

func (r *KeyRingIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("keyringiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch KeyRingIamPolicy")
...
func (r *KeyRingIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.KeyRingIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.KeyRingIamPolicy{}.APIVersion, kmsv1alpha1.KeyRingIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.KeyRingIamPolicy{}).
