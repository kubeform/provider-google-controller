...
// KeyRingImportJobReconciler reconciles a KeyRingImportJob object
type KeyRingImportJobReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.google.kubeform.com,resources=keyringimportjobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.google.kubeform.com,resources=keyringimportjobs/status,verbs=get;update;patch

func (r *KeyRingImportJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("keyringimportjob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch KeyRingImportJob")
...
func (r *KeyRingImportJobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.KeyRingImportJob{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.KeyRingImportJob{}.APIVersion, kmsv1alpha1.KeyRingImportJob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.KeyRingImportJob{}).
