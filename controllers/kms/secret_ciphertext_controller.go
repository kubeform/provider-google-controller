...
// SecretCiphertextReconciler reconciles a SecretCiphertext object
type SecretCiphertextReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.google.kubeform.com,resources=secretciphertexts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.google.kubeform.com,resources=secretciphertexts/status,verbs=get;update;patch

func (r *SecretCiphertextReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("secretciphertext", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SecretCiphertext")
...
func (r *SecretCiphertextReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.SecretCiphertext{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.SecretCiphertext{}.APIVersion, kmsv1alpha1.SecretCiphertext{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.SecretCiphertext{}).
...
func (r *SecretCiphertextReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for SecretCiphertext")
			return nil
		}
		...
		resourceList := &kmsv1alpha1.SecretCiphertextList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SecretCiphertext")
			return nil
		}
