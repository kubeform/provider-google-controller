...
// BillingAccountSinkReconciler reconciles a BillingAccountSink object
type BillingAccountSinkReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logging.google.kubeform.com,resources=billingaccountsinks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logging.google.kubeform.com,resources=billingaccountsinks/status,verbs=get;update;patch

func (r *BillingAccountSinkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("billingaccountsink", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BillingAccountSink")
...
func (r *BillingAccountSinkReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loggingv1alpha1.BillingAccountSink{}); err != nil {
			klog.Error(err, "unable to set up auditor", loggingv1alpha1.BillingAccountSink{}.APIVersion, loggingv1alpha1.BillingAccountSink{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loggingv1alpha1.BillingAccountSink{}).
