...
// FolderSinkReconciler reconciles a FolderSink object
type FolderSinkReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logging.google.kubeform.com,resources=foldersinks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logging.google.kubeform.com,resources=foldersinks/status,verbs=get;update;patch

func (r *FolderSinkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("foldersink", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FolderSink")
...
func (r *FolderSinkReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loggingv1alpha1.FolderSink{}); err != nil {
			klog.Error(err, "unable to set up auditor", loggingv1alpha1.FolderSink{}.APIVersion, loggingv1alpha1.FolderSink{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loggingv1alpha1.FolderSink{}).
