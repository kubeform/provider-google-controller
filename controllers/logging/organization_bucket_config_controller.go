...
// OrganizationBucketConfigReconciler reconciles a OrganizationBucketConfig object
type OrganizationBucketConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logging.google.kubeform.com,resources=organizationbucketconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logging.google.kubeform.com,resources=organizationbucketconfigs/status,verbs=get;update;patch

func (r *OrganizationBucketConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("organizationbucketconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OrganizationBucketConfig")
...
func (r *OrganizationBucketConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loggingv1alpha1.OrganizationBucketConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", loggingv1alpha1.OrganizationBucketConfig{}.APIVersion, loggingv1alpha1.OrganizationBucketConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loggingv1alpha1.OrganizationBucketConfig{}).
