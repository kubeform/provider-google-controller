...
// OrganizationExclusionReconciler reconciles a OrganizationExclusion object
type OrganizationExclusionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logging.google.kubeform.com,resources=organizationexclusions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logging.google.kubeform.com,resources=organizationexclusions/status,verbs=get;update;patch

func (r *OrganizationExclusionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("organizationexclusion", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OrganizationExclusion")
...
func (r *OrganizationExclusionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loggingv1alpha1.OrganizationExclusion{}); err != nil {
			klog.Error(err, "unable to set up auditor", loggingv1alpha1.OrganizationExclusion{}.APIVersion, loggingv1alpha1.OrganizationExclusion{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loggingv1alpha1.OrganizationExclusion{}).
