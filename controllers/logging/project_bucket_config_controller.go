...
// ProjectBucketConfigReconciler reconciles a ProjectBucketConfig object
type ProjectBucketConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logging.google.kubeform.com,resources=projectbucketconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logging.google.kubeform.com,resources=projectbucketconfigs/status,verbs=get;update;patch

func (r *ProjectBucketConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("projectbucketconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProjectBucketConfig")
...
func (r *ProjectBucketConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loggingv1alpha1.ProjectBucketConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", loggingv1alpha1.ProjectBucketConfig{}.APIVersion, loggingv1alpha1.ProjectBucketConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loggingv1alpha1.ProjectBucketConfig{}).
