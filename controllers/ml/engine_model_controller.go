...
// EngineModelReconciler reconciles a EngineModel object
type EngineModelReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ml.google.kubeform.com,resources=enginemodels,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ml.google.kubeform.com,resources=enginemodels/status,verbs=get;update;patch

func (r *EngineModelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("enginemodel", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EngineModel")
...
func (r *EngineModelReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mlv1alpha1.EngineModel{}); err != nil {
			klog.Error(err, "unable to set up auditor", mlv1alpha1.EngineModel{}.APIVersion, mlv1alpha1.EngineModel{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mlv1alpha1.EngineModel{}).
