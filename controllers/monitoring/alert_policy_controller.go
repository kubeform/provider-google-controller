...
// +kubebuilder:rbac:groups=monitoring.google.kubeform.com,resources=alertpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=monitoring.google.kubeform.com,resources=alertpolicies/status,verbs=get;update;patch

func (r *AlertPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alertpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AlertPolicy")
...
func (r *AlertPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &monitoringv1alpha1.AlertPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", monitoringv1alpha1.AlertPolicy{}.APIVersion, monitoringv1alpha1.AlertPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&monitoringv1alpha1.AlertPolicy{}).
