...
// MetricDescriptorReconciler reconciles a MetricDescriptor object
type MetricDescriptorReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=monitoring.google.kubeform.com,resources=metricdescriptors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=monitoring.google.kubeform.com,resources=metricdescriptors/status,verbs=get;update;patch

func (r *MetricDescriptorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("metricdescriptor", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MetricDescriptor")
...
func (r *MetricDescriptorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &monitoringv1alpha1.MetricDescriptor{}); err != nil {
			klog.Error(err, "unable to set up auditor", monitoringv1alpha1.MetricDescriptor{}.APIVersion, monitoringv1alpha1.MetricDescriptor{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&monitoringv1alpha1.MetricDescriptor{}).
