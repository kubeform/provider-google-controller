...
// +kubebuilder:rbac:groups=monitoring.google.kubeform.com,resources=notificationchannels,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=monitoring.google.kubeform.com,resources=notificationchannels/status,verbs=get;update;patch

func (r *NotificationChannelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("notificationchannel", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NotificationChannel")
...
func (r *NotificationChannelReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &monitoringv1alpha1.NotificationChannel{}); err != nil {
			klog.Error(err, "unable to set up auditor", monitoringv1alpha1.NotificationChannel{}.APIVersion, monitoringv1alpha1.NotificationChannel{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&monitoringv1alpha1.NotificationChannel{}).
...
func (r *NotificationChannelReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for NotificationChannel")
			return nil
		}
		...
		resourceList := &monitoringv1alpha1.NotificationChannelList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list NotificationChannel")
			return nil
		}
