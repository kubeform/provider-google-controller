...
// UptimeCheckConfigReconciler reconciles a UptimeCheckConfig object
type UptimeCheckConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=monitoring.google.kubeform.com,resources=uptimecheckconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=monitoring.google.kubeform.com,resources=uptimecheckconfigs/status,verbs=get;update;patch

func (r *UptimeCheckConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("uptimecheckconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UptimeCheckConfig")
...
func (r *UptimeCheckConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &monitoringv1alpha1.UptimeCheckConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", monitoringv1alpha1.UptimeCheckConfig{}.APIVersion, monitoringv1alpha1.UptimeCheckConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&monitoringv1alpha1.UptimeCheckConfig{}).
...
func (r *UptimeCheckConfigReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for UptimeCheckConfig")
			return nil
		}
		...
		resourceList := &monitoringv1alpha1.UptimeCheckConfigList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list UptimeCheckConfig")
			return nil
		}
