...
// ConnectivityHubReconciler reconciles a ConnectivityHub object
type ConnectivityHubReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=network.google.kubeform.com,resources=connectivityhubs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.google.kubeform.com,resources=connectivityhubs/status,verbs=get;update;patch

func (r *ConnectivityHubReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectivityhub", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectivityHub")
...
func (r *ConnectivityHubReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.ConnectivityHub{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.ConnectivityHub{}.APIVersion, networkv1alpha1.ConnectivityHub{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.ConnectivityHub{}).
