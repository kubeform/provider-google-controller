...
// ConnectivitySpokeReconciler reconciles a ConnectivitySpoke object
type ConnectivitySpokeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=network.google.kubeform.com,resources=connectivityspokes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.google.kubeform.com,resources=connectivityspokes/status,verbs=get;update;patch

func (r *ConnectivitySpokeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectivityspoke", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectivitySpoke")
...
func (r *ConnectivitySpokeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.ConnectivitySpoke{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.ConnectivitySpoke{}.APIVersion, networkv1alpha1.ConnectivitySpoke{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.ConnectivitySpoke{}).
