...
// ServicesEdgeCacheKeysetReconciler reconciles a ServicesEdgeCacheKeyset object
type ServicesEdgeCacheKeysetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=network.google.kubeform.com,resources=servicesedgecachekeysets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.google.kubeform.com,resources=servicesedgecachekeysets/status,verbs=get;update;patch

func (r *ServicesEdgeCacheKeysetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicesedgecachekeyset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServicesEdgeCacheKeyset")
...
func (r *ServicesEdgeCacheKeysetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.ServicesEdgeCacheKeyset{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.ServicesEdgeCacheKeyset{}.APIVersion, networkv1alpha1.ServicesEdgeCacheKeyset{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.ServicesEdgeCacheKeyset{}).
...
func (r *ServicesEdgeCacheKeysetReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for ServicesEdgeCacheKeyset")
			return nil
		}
		...
		resourceList := &networkv1alpha1.ServicesEdgeCacheKeysetList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ServicesEdgeCacheKeyset")
			return nil
		}
