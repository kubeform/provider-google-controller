...
// ServicesEdgeCacheServiceReconciler reconciles a ServicesEdgeCacheService object
type ServicesEdgeCacheServiceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=network.google.kubeform.com,resources=servicesedgecacheservices,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.google.kubeform.com,resources=servicesedgecacheservices/status,verbs=get;update;patch

func (r *ServicesEdgeCacheServiceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicesedgecacheservice", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServicesEdgeCacheService")
...
func (r *ServicesEdgeCacheServiceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.ServicesEdgeCacheService{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.ServicesEdgeCacheService{}.APIVersion, networkv1alpha1.ServicesEdgeCacheService{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.ServicesEdgeCacheService{}).
