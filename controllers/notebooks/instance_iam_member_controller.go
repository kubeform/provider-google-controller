...
// InstanceIamMemberReconciler reconciles a InstanceIamMember object
type InstanceIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=notebooks.google.kubeform.com,resources=instanceiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=notebooks.google.kubeform.com,resources=instanceiammembers/status,verbs=get;update;patch

func (r *InstanceIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("instanceiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InstanceIamMember")
...
func (r *InstanceIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &notebooksv1alpha1.InstanceIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", notebooksv1alpha1.InstanceIamMember{}.APIVersion, notebooksv1alpha1.InstanceIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&notebooksv1alpha1.InstanceIamMember{}).
