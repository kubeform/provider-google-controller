...
// RuntimeIamBindingReconciler reconciles a RuntimeIamBinding object
type RuntimeIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=notebooks.google.kubeform.com,resources=runtimeiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=notebooks.google.kubeform.com,resources=runtimeiambindings/status,verbs=get;update;patch

func (r *RuntimeIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("runtimeiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RuntimeIamBinding")
...
func (r *RuntimeIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &notebooksv1alpha1.RuntimeIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", notebooksv1alpha1.RuntimeIamBinding{}.APIVersion, notebooksv1alpha1.RuntimeIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&notebooksv1alpha1.RuntimeIamBinding{}).
