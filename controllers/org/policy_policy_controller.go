...
// PolicyPolicyReconciler reconciles a PolicyPolicy object
type PolicyPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=org.google.kubeform.com,resources=policypolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=org.google.kubeform.com,resources=policypolicies/status,verbs=get;update;patch

func (r *PolicyPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("policypolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PolicyPolicy")
...
func (r *PolicyPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &orgv1alpha1.PolicyPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", orgv1alpha1.PolicyPolicy{}.APIVersion, orgv1alpha1.PolicyPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&orgv1alpha1.PolicyPolicy{}).
