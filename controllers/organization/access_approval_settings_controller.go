...
// AccessApprovalSettingsReconciler reconciles a AccessApprovalSettings object
type AccessApprovalSettingsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=organization.google.kubeform.com,resources=accessapprovalsettings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=organization.google.kubeform.com,resources=accessapprovalsettings/status,verbs=get;update;patch

func (r *AccessApprovalSettingsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accessapprovalsettings", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccessApprovalSettings")
...
func (r *AccessApprovalSettingsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &organizationv1alpha1.AccessApprovalSettings{}); err != nil {
			klog.Error(err, "unable to set up auditor", organizationv1alpha1.AccessApprovalSettings{}.APIVersion, organizationv1alpha1.AccessApprovalSettings{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&organizationv1alpha1.AccessApprovalSettings{}).
