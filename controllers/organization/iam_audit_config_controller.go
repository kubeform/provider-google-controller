...
// IamAuditConfigReconciler reconciles a IamAuditConfig object
type IamAuditConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=organization.google.kubeform.com,resources=iamauditconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=organization.google.kubeform.com,resources=iamauditconfigs/status,verbs=get;update;patch

func (r *IamAuditConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("iamauditconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IamAuditConfig")
...
func (r *IamAuditConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &organizationv1alpha1.IamAuditConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", organizationv1alpha1.IamAuditConfig{}.APIVersion, organizationv1alpha1.IamAuditConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&organizationv1alpha1.IamAuditConfig{}).
