...
// ConfigOsPolicyAssignmentReconciler reconciles a ConfigOsPolicyAssignment object
type ConfigOsPolicyAssignmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=os.google.kubeform.com,resources=configospolicyassignments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=os.google.kubeform.com,resources=configospolicyassignments/status,verbs=get;update;patch

func (r *ConfigOsPolicyAssignmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("configospolicyassignment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConfigOsPolicyAssignment")
...
func (r *ConfigOsPolicyAssignmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &osv1alpha1.ConfigOsPolicyAssignment{}); err != nil {
			klog.Error(err, "unable to set up auditor", osv1alpha1.ConfigOsPolicyAssignment{}.APIVersion, osv1alpha1.ConfigOsPolicyAssignment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&osv1alpha1.ConfigOsPolicyAssignment{}).
