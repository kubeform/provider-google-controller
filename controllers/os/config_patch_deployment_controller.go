...
// ConfigPatchDeploymentReconciler reconciles a ConfigPatchDeployment object
type ConfigPatchDeploymentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=os.google.kubeform.com,resources=configpatchdeployments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=os.google.kubeform.com,resources=configpatchdeployments/status,verbs=get;update;patch

func (r *ConfigPatchDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("configpatchdeployment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConfigPatchDeployment")
...
func (r *ConfigPatchDeploymentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &osv1alpha1.ConfigPatchDeployment{}); err != nil {
			klog.Error(err, "unable to set up auditor", osv1alpha1.ConfigPatchDeployment{}.APIVersion, osv1alpha1.ConfigPatchDeployment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&osv1alpha1.ConfigPatchDeployment{}).
