...
// LoginSSHPublicKeyReconciler reconciles a LoginSSHPublicKey object
type LoginSSHPublicKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=os.google.kubeform.com,resources=loginsshpublickeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=os.google.kubeform.com,resources=loginsshpublickeys/status,verbs=get;update;patch

func (r *LoginSSHPublicKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("loginsshpublickey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LoginSSHPublicKey")
...
func (r *LoginSSHPublicKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &osv1alpha1.LoginSSHPublicKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", osv1alpha1.LoginSSHPublicKey{}.APIVersion, osv1alpha1.LoginSSHPublicKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&osv1alpha1.LoginSSHPublicKey{}).
