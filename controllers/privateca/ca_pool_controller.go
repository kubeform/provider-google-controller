...
// CaPoolReconciler reconciles a CaPool object
type CaPoolReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=privateca.google.kubeform.com,resources=capools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=privateca.google.kubeform.com,resources=capools/status,verbs=get;update;patch

func (r *CaPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("capool", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CaPool")
...
func (r *CaPoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatecav1alpha1.CaPool{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatecav1alpha1.CaPool{}.APIVersion, privatecav1alpha1.CaPool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatecav1alpha1.CaPool{}).
