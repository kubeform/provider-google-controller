...
// CaPoolIamBindingReconciler reconciles a CaPoolIamBinding object
type CaPoolIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=privateca.google.kubeform.com,resources=capooliambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=privateca.google.kubeform.com,resources=capooliambindings/status,verbs=get;update;patch

func (r *CaPoolIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("capooliambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CaPoolIamBinding")
...
func (r *CaPoolIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatecav1alpha1.CaPoolIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatecav1alpha1.CaPoolIamBinding{}.APIVersion, privatecav1alpha1.CaPoolIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatecav1alpha1.CaPoolIamBinding{}).
