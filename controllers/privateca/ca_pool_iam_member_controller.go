...
// CaPoolIamMemberReconciler reconciles a CaPoolIamMember object
type CaPoolIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=privateca.google.kubeform.com,resources=capooliammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=privateca.google.kubeform.com,resources=capooliammembers/status,verbs=get;update;patch

func (r *CaPoolIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("capooliammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CaPoolIamMember")
...
func (r *CaPoolIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatecav1alpha1.CaPoolIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatecav1alpha1.CaPoolIamMember{}.APIVersion, privatecav1alpha1.CaPoolIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatecav1alpha1.CaPoolIamMember{}).
