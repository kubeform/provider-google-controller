...
// CaPoolIamPolicyReconciler reconciles a CaPoolIamPolicy object
type CaPoolIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=privateca.google.kubeform.com,resources=capooliampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=privateca.google.kubeform.com,resources=capooliampolicies/status,verbs=get;update;patch

func (r *CaPoolIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("capooliampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CaPoolIamPolicy")
...
func (r *CaPoolIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatecav1alpha1.CaPoolIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatecav1alpha1.CaPoolIamPolicy{}.APIVersion, privatecav1alpha1.CaPoolIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatecav1alpha1.CaPoolIamPolicy{}).
