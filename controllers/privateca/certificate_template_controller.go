...
// CertificateTemplateReconciler reconciles a CertificateTemplate object
type CertificateTemplateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=privateca.google.kubeform.com,resources=certificatetemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=privateca.google.kubeform.com,resources=certificatetemplates/status,verbs=get;update;patch

func (r *CertificateTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("certificatetemplate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CertificateTemplate")
...
func (r *CertificateTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatecav1alpha1.CertificateTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatecav1alpha1.CertificateTemplate{}.APIVersion, privatecav1alpha1.CertificateTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatecav1alpha1.CertificateTemplate{}).
