...
// DefaultServiceAccountsReconciler reconciles a DefaultServiceAccounts object
type DefaultServiceAccountsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=project.google.kubeform.com,resources=defaultserviceaccounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=project.google.kubeform.com,resources=defaultserviceaccounts/status,verbs=get;update;patch

func (r *DefaultServiceAccountsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("defaultserviceaccounts", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DefaultServiceAccounts")
...
func (r *DefaultServiceAccountsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &projectv1alpha1.DefaultServiceAccounts{}); err != nil {
			klog.Error(err, "unable to set up auditor", projectv1alpha1.DefaultServiceAccounts{}.APIVersion, projectv1alpha1.DefaultServiceAccounts{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&projectv1alpha1.DefaultServiceAccounts{}).
