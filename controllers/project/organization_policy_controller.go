...
// OrganizationPolicyReconciler reconciles a OrganizationPolicy object
type OrganizationPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=project.google.kubeform.com,resources=organizationpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=project.google.kubeform.com,resources=organizationpolicies/status,verbs=get;update;patch

func (r *OrganizationPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("organizationpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OrganizationPolicy")
...
func (r *OrganizationPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &projectv1alpha1.OrganizationPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", projectv1alpha1.OrganizationPolicy{}.APIVersion, projectv1alpha1.OrganizationPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&projectv1alpha1.OrganizationPolicy{}).
