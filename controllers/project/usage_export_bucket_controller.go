...
// UsageExportBucketReconciler reconciles a UsageExportBucket object
type UsageExportBucketReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=project.google.kubeform.com,resources=usageexportbuckets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=project.google.kubeform.com,resources=usageexportbuckets/status,verbs=get;update;patch

func (r *UsageExportBucketReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("usageexportbucket", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UsageExportBucket")
...
func (r *UsageExportBucketReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &projectv1alpha1.UsageExportBucket{}); err != nil {
			klog.Error(err, "unable to set up auditor", projectv1alpha1.UsageExportBucket{}.APIVersion, projectv1alpha1.UsageExportBucket{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&projectv1alpha1.UsageExportBucket{}).
