...
// LiteTopicReconciler reconciles a LiteTopic object
type LiteTopicReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=pubsub.google.kubeform.com,resources=litetopics,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pubsub.google.kubeform.com,resources=litetopics/status,verbs=get;update;patch

func (r *LiteTopicReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("litetopic", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LiteTopic")
...
func (r *LiteTopicReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &pubsubv1alpha1.LiteTopic{}); err != nil {
			klog.Error(err, "unable to set up auditor", pubsubv1alpha1.LiteTopic{}.APIVersion, pubsubv1alpha1.LiteTopic{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&pubsubv1alpha1.LiteTopic{}).
