...
// SubscriptionIamMemberReconciler reconciles a SubscriptionIamMember object
type SubscriptionIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=pubsub.google.kubeform.com,resources=subscriptioniammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pubsub.google.kubeform.com,resources=subscriptioniammembers/status,verbs=get;update;patch

func (r *SubscriptionIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("subscriptioniammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SubscriptionIamMember")
...
func (r *SubscriptionIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &pubsubv1alpha1.SubscriptionIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", pubsubv1alpha1.SubscriptionIamMember{}.APIVersion, pubsubv1alpha1.SubscriptionIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&pubsubv1alpha1.SubscriptionIamMember{}).
