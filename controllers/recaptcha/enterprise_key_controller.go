...
// EnterpriseKeyReconciler reconciles a EnterpriseKey object
type EnterpriseKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=recaptcha.google.kubeform.com,resources=enterprisekeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=recaptcha.google.kubeform.com,resources=enterprisekeys/status,verbs=get;update;patch

func (r *EnterpriseKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("enterprisekey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EnterpriseKey")
...
func (r *EnterpriseKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &recaptchav1alpha1.EnterpriseKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", recaptchav1alpha1.EnterpriseKey{}.APIVersion, recaptchav1alpha1.EnterpriseKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&recaptchav1alpha1.EnterpriseKey{}).
