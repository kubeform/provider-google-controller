...
// ManagerLienReconciler reconciles a ManagerLien object
type ManagerLienReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=resource.google.kubeform.com,resources=managerliens,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=resource.google.kubeform.com,resources=managerliens/status,verbs=get;update;patch

func (r *ManagerLienReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managerlien", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagerLien")
...
func (r *ManagerLienReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &resourcev1alpha1.ManagerLien{}); err != nil {
			klog.Error(err, "unable to set up auditor", resourcev1alpha1.ManagerLien{}.APIVersion, resourcev1alpha1.ManagerLien{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&resourcev1alpha1.ManagerLien{}).
