...
// ConfigIamBindingReconciler reconciles a ConfigIamBinding object
type ConfigIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=runtimeconfig.google.kubeform.com,resources=configiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=runtimeconfig.google.kubeform.com,resources=configiambindings/status,verbs=get;update;patch

func (r *ConfigIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("configiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConfigIamBinding")
...
func (r *ConfigIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &runtimeconfigv1alpha1.ConfigIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", runtimeconfigv1alpha1.ConfigIamBinding{}.APIVersion, runtimeconfigv1alpha1.ConfigIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&runtimeconfigv1alpha1.ConfigIamBinding{}).
