...
// ConfigIamMemberReconciler reconciles a ConfigIamMember object
type ConfigIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=runtimeconfig.google.kubeform.com,resources=configiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=runtimeconfig.google.kubeform.com,resources=configiammembers/status,verbs=get;update;patch

func (r *ConfigIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("configiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConfigIamMember")
...
func (r *ConfigIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &runtimeconfigv1alpha1.ConfigIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", runtimeconfigv1alpha1.ConfigIamMember{}.APIVersion, runtimeconfigv1alpha1.ConfigIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&runtimeconfigv1alpha1.ConfigIamMember{}).
