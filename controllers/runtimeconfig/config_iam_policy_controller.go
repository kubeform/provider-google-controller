...
// ConfigIamPolicyReconciler reconciles a ConfigIamPolicy object
type ConfigIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=runtimeconfig.google.kubeform.com,resources=configiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=runtimeconfig.google.kubeform.com,resources=configiampolicies/status,verbs=get;update;patch

func (r *ConfigIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("configiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConfigIamPolicy")
...
func (r *ConfigIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &runtimeconfigv1alpha1.ConfigIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", runtimeconfigv1alpha1.ConfigIamPolicy{}.APIVersion, runtimeconfigv1alpha1.ConfigIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&runtimeconfigv1alpha1.ConfigIamPolicy{}).
