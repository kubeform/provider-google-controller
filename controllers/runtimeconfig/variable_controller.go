...
// VariableReconciler reconciles a Variable object
type VariableReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=runtimeconfig.google.kubeform.com,resources=variables,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=runtimeconfig.google.kubeform.com,resources=variables/status,verbs=get;update;patch

func (r *VariableReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("variable", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Variable")
...
func (r *VariableReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &runtimeconfigv1alpha1.Variable{}); err != nil {
			klog.Error(err, "unable to set up auditor", runtimeconfigv1alpha1.Variable{}.APIVersion, runtimeconfigv1alpha1.Variable{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&runtimeconfigv1alpha1.Variable{}).
...
func (r *VariableReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Variable")
			return nil
		}
		...
		resourceList := &runtimeconfigv1alpha1.VariableList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Variable")
			return nil
		}
