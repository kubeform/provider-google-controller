...
// ManagerSecretIamBindingReconciler reconciles a ManagerSecretIamBinding object
type ManagerSecretIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=secret.google.kubeform.com,resources=managersecretiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=secret.google.kubeform.com,resources=managersecretiambindings/status,verbs=get;update;patch

func (r *ManagerSecretIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managersecretiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagerSecretIamBinding")
...
func (r *ManagerSecretIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &secretv1alpha1.ManagerSecretIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", secretv1alpha1.ManagerSecretIamBinding{}.APIVersion, secretv1alpha1.ManagerSecretIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&secretv1alpha1.ManagerSecretIamBinding{}).
