...
// ManagerSecretIamMemberReconciler reconciles a ManagerSecretIamMember object
type ManagerSecretIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=secret.google.kubeform.com,resources=managersecretiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=secret.google.kubeform.com,resources=managersecretiammembers/status,verbs=get;update;patch

func (r *ManagerSecretIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managersecretiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagerSecretIamMember")
...
func (r *ManagerSecretIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &secretv1alpha1.ManagerSecretIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", secretv1alpha1.ManagerSecretIamMember{}.APIVersion, secretv1alpha1.ManagerSecretIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&secretv1alpha1.ManagerSecretIamMember{}).
