...
// ManagerSecretIamPolicyReconciler reconciles a ManagerSecretIamPolicy object
type ManagerSecretIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=secret.google.kubeform.com,resources=managersecretiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=secret.google.kubeform.com,resources=managersecretiampolicies/status,verbs=get;update;patch

func (r *ManagerSecretIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managersecretiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagerSecretIamPolicy")
...
func (r *ManagerSecretIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &secretv1alpha1.ManagerSecretIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", secretv1alpha1.ManagerSecretIamPolicy{}.APIVersion, secretv1alpha1.ManagerSecretIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&secretv1alpha1.ManagerSecretIamPolicy{}).
