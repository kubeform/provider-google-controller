...
// ManagerSecretVersionReconciler reconciles a ManagerSecretVersion object
type ManagerSecretVersionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=secret.google.kubeform.com,resources=managersecretversions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=secret.google.kubeform.com,resources=managersecretversions/status,verbs=get;update;patch

func (r *ManagerSecretVersionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managersecretversion", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagerSecretVersion")
...
func (r *ManagerSecretVersionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &secretv1alpha1.ManagerSecretVersion{}); err != nil {
			klog.Error(err, "unable to set up auditor", secretv1alpha1.ManagerSecretVersion{}.APIVersion, secretv1alpha1.ManagerSecretVersion{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&secretv1alpha1.ManagerSecretVersion{}).
...
func (r *ManagerSecretVersionReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ManagerSecretVersion")
			return nil
		}
		...
		resourceList := &secretv1alpha1.ManagerSecretVersionList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ManagerSecretVersion")
			return nil
		}
