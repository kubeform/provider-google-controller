...
// AccountIamBindingReconciler reconciles a AccountIamBinding object
type AccountIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=service.google.kubeform.com,resources=accountiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.google.kubeform.com,resources=accountiambindings/status,verbs=get;update;patch

func (r *AccountIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accountiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccountIamBinding")
...
func (r *AccountIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.AccountIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.AccountIamBinding{}.APIVersion, servicev1alpha1.AccountIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.AccountIamBinding{}).
