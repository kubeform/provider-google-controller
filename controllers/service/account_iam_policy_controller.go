...
// AccountIamPolicyReconciler reconciles a AccountIamPolicy object
type AccountIamPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=service.google.kubeform.com,resources=accountiampolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.google.kubeform.com,resources=accountiampolicies/status,verbs=get;update;patch

func (r *AccountIamPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accountiampolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccountIamPolicy")
...
func (r *AccountIamPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.AccountIamPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.AccountIamPolicy{}.APIVersion, servicev1alpha1.AccountIamPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.AccountIamPolicy{}).
