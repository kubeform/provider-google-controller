...
// AccountKeyReconciler reconciles a AccountKey object
type AccountKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=service.google.kubeform.com,resources=accountkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.google.kubeform.com,resources=accountkeys/status,verbs=get;update;patch

func (r *AccountKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accountkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccountKey")
...
func (r *AccountKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.AccountKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.AccountKey{}.APIVersion, servicev1alpha1.AccountKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.AccountKey{}).
...
func (r *AccountKeyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for AccountKey")
			return nil
		}
		...
		resourceList := &servicev1alpha1.AccountKeyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AccountKey")
			return nil
		}
