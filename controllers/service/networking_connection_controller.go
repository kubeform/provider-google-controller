...
// NetworkingConnectionReconciler reconciles a NetworkingConnection object
type NetworkingConnectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=service.google.kubeform.com,resources=networkingconnections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.google.kubeform.com,resources=networkingconnections/status,verbs=get;update;patch

func (r *NetworkingConnectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkingconnection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkingConnection")
...
func (r *NetworkingConnectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.NetworkingConnection{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.NetworkingConnection{}.APIVersion, servicev1alpha1.NetworkingConnection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.NetworkingConnection{}).
