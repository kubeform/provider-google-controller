...
// NetworkingPeeredDNSDomainReconciler reconciles a NetworkingPeeredDNSDomain object
type NetworkingPeeredDNSDomainReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=service.google.kubeform.com,resources=networkingpeereddnsdomains,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.google.kubeform.com,resources=networkingpeereddnsdomains/status,verbs=get;update;patch

func (r *NetworkingPeeredDNSDomainReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkingpeereddnsdomain", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkingPeeredDNSDomain")
...
func (r *NetworkingPeeredDNSDomainReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.NetworkingPeeredDNSDomain{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.NetworkingPeeredDNSDomain{}.APIVersion, servicev1alpha1.NetworkingPeeredDNSDomain{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.NetworkingPeeredDNSDomain{}).
