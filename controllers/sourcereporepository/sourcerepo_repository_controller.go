...
// SourcerepoRepositoryReconciler reconciles a SourcerepoRepository object
type SourcerepoRepositoryReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sourcereporepository.google.kubeform.com,resources=sourcereporepositories,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sourcereporepository.google.kubeform.com,resources=sourcereporepositories/status,verbs=get;update;patch

func (r *SourcerepoRepositoryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sourcereporepository", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SourcerepoRepository")
...
func (r *SourcerepoRepositoryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sourcereporepositoryv1alpha1.SourcerepoRepository{}); err != nil {
			klog.Error(err, "unable to set up auditor", sourcereporepositoryv1alpha1.SourcerepoRepository{}.APIVersion, sourcereporepositoryv1alpha1.SourcerepoRepository{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sourcereporepositoryv1alpha1.SourcerepoRepository{}).
