...
// DatabaseIamBindingReconciler reconciles a DatabaseIamBinding object
type DatabaseIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=spanner.google.kubeform.com,resources=databaseiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=spanner.google.kubeform.com,resources=databaseiambindings/status,verbs=get;update;patch

func (r *DatabaseIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("databaseiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DatabaseIamBinding")
...
func (r *DatabaseIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &spannerv1alpha1.DatabaseIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", spannerv1alpha1.DatabaseIamBinding{}.APIVersion, spannerv1alpha1.DatabaseIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&spannerv1alpha1.DatabaseIamBinding{}).
