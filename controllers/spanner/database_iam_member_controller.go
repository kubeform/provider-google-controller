...
// DatabaseIamMemberReconciler reconciles a DatabaseIamMember object
type DatabaseIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=spanner.google.kubeform.com,resources=databaseiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=spanner.google.kubeform.com,resources=databaseiammembers/status,verbs=get;update;patch

func (r *DatabaseIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("databaseiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DatabaseIamMember")
...
func (r *DatabaseIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &spannerv1alpha1.DatabaseIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", spannerv1alpha1.DatabaseIamMember{}.APIVersion, spannerv1alpha1.DatabaseIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&spannerv1alpha1.DatabaseIamMember{}).
