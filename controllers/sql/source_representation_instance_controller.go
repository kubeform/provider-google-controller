...
// SourceRepresentationInstanceReconciler reconciles a SourceRepresentationInstance object
type SourceRepresentationInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sql.google.kubeform.com,resources=sourcerepresentationinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sql.google.kubeform.com,resources=sourcerepresentationinstances/status,verbs=get;update;patch

func (r *SourceRepresentationInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sourcerepresentationinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SourceRepresentationInstance")
...
func (r *SourceRepresentationInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sqlv1alpha1.SourceRepresentationInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", sqlv1alpha1.SourceRepresentationInstance{}.APIVersion, sqlv1alpha1.SourceRepresentationInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sqlv1alpha1.SourceRepresentationInstance{}).
