...
// SslCertReconciler reconciles a SslCert object
type SslCertReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sql.google.kubeform.com,resources=sslcerts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sql.google.kubeform.com,resources=sslcerts/status,verbs=get;update;patch

func (r *SslCertReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sslcert", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SslCert")
...
func (r *SslCertReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sqlv1alpha1.SslCert{}); err != nil {
			klog.Error(err, "unable to set up auditor", sqlv1alpha1.SslCert{}.APIVersion, sqlv1alpha1.SslCert{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sqlv1alpha1.SslCert{}).
...
func (r *SslCertReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for SslCert")
			return nil
		}
		...
		resourceList := &sqlv1alpha1.SslCertList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SslCert")
			return nil
		}
