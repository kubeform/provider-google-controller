...
// BucketAccessControlReconciler reconciles a BucketAccessControl object
type BucketAccessControlReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storage.google.kubeform.com,resources=bucketaccesscontrols,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.google.kubeform.com,resources=bucketaccesscontrols/status,verbs=get;update;patch

func (r *BucketAccessControlReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bucketaccesscontrol", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BucketAccessControl")
...
func (r *BucketAccessControlReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.BucketAccessControl{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.BucketAccessControl{}.APIVersion, storagev1alpha1.BucketAccessControl{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.BucketAccessControl{}).
