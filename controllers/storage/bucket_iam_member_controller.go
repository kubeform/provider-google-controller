...
// BucketIamMemberReconciler reconciles a BucketIamMember object
type BucketIamMemberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storage.google.kubeform.com,resources=bucketiammembers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.google.kubeform.com,resources=bucketiammembers/status,verbs=get;update;patch

func (r *BucketIamMemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bucketiammember", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BucketIamMember")
...
func (r *BucketIamMemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.BucketIamMember{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.BucketIamMember{}.APIVersion, storagev1alpha1.BucketIamMember{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.BucketIamMember{}).
