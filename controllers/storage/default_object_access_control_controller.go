...
// DefaultObjectAccessControlReconciler reconciles a DefaultObjectAccessControl object
type DefaultObjectAccessControlReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storage.google.kubeform.com,resources=defaultobjectaccesscontrols,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.google.kubeform.com,resources=defaultobjectaccesscontrols/status,verbs=get;update;patch

func (r *DefaultObjectAccessControlReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("defaultobjectaccesscontrol", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DefaultObjectAccessControl")
...
func (r *DefaultObjectAccessControlReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.DefaultObjectAccessControl{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.DefaultObjectAccessControl{}.APIVersion, storagev1alpha1.DefaultObjectAccessControl{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.DefaultObjectAccessControl{}).
