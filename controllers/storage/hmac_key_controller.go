...
// HmacKeyReconciler reconciles a HmacKey object
type HmacKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storage.google.kubeform.com,resources=hmackeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.google.kubeform.com,resources=hmackeys/status,verbs=get;update;patch

func (r *HmacKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hmackey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HmacKey")
...
func (r *HmacKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.HmacKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.HmacKey{}.APIVersion, storagev1alpha1.HmacKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.HmacKey{}).
...
func (r *HmacKeyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for HmacKey")
			return nil
		}
		...
		resourceList := &storagev1alpha1.HmacKeyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list HmacKey")
			return nil
		}
