...
// TransferJobReconciler reconciles a TransferJob object
type TransferJobReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storage.google.kubeform.com,resources=transferjobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.google.kubeform.com,resources=transferjobs/status,verbs=get;update;patch

func (r *TransferJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("transferjob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TransferJob")
...
func (r *TransferJobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.TransferJob{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.TransferJob{}.APIVersion, storagev1alpha1.TransferJob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.TransferJob{}).
...
func (r *TransferJobReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for TransferJob")
			return nil
		}
		...
		resourceList := &storagev1alpha1.TransferJobList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list TransferJob")
			return nil
		}
