...
// TagBindingReconciler reconciles a TagBinding object
type TagBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=tags.google.kubeform.com,resources=tagbindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=tags.google.kubeform.com,resources=tagbindings/status,verbs=get;update;patch

func (r *TagBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tagbinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TagBinding")
...
func (r *TagBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &tagsv1alpha1.TagBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", tagsv1alpha1.TagBinding{}.APIVersion, tagsv1alpha1.TagBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&tagsv1alpha1.TagBinding{}).
