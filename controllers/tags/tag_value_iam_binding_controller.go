...
// TagValueIamBindingReconciler reconciles a TagValueIamBinding object
type TagValueIamBindingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=tags.google.kubeform.com,resources=tagvalueiambindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=tags.google.kubeform.com,resources=tagvalueiambindings/status,verbs=get;update;patch

func (r *TagValueIamBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tagvalueiambinding", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TagValueIamBinding")
...
func (r *TagValueIamBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &tagsv1alpha1.TagValueIamBinding{}); err != nil {
			klog.Error(err, "unable to set up auditor", tagsv1alpha1.TagValueIamBinding{}.APIVersion, tagsv1alpha1.TagValueIamBinding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&tagsv1alpha1.TagValueIamBinding{}).
