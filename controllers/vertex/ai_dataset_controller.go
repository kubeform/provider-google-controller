...
// AiDatasetReconciler reconciles a AiDataset object
type AiDatasetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vertex.google.kubeform.com,resources=aidatasets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vertex.google.kubeform.com,resources=aidatasets/status,verbs=get;update;patch

func (r *AiDatasetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("aidataset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AiDataset")
...
func (r *AiDatasetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vertexv1alpha1.AiDataset{}); err != nil {
			klog.Error(err, "unable to set up auditor", vertexv1alpha1.AiDataset{}.APIVersion, vertexv1alpha1.AiDataset{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vertexv1alpha1.AiDataset{}).
