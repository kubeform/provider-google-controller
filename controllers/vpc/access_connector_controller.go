...
// AccessConnectorReconciler reconciles a AccessConnector object
type AccessConnectorReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.google.kubeform.com,resources=accessconnectors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.google.kubeform.com,resources=accessconnectors/status,verbs=get;update;patch

func (r *AccessConnectorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accessconnector", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccessConnector")
...
func (r *AccessConnectorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.AccessConnector{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.AccessConnector{}.APIVersion, vpcv1alpha1.AccessConnector{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.AccessConnector{}).
